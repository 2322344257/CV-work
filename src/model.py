...
#     def __init__(self):
#         super(Detector, self).__init__()
#         # Replace EfficientNet with ResNet
#         self.net = models.resnet50(pretrained=True)
#         # Assuming the ResNet model has the same number of output features as EfficientNet
#         self.fc = nn.Linear(self.net.fc.in_features, 16)#将2改为16
#         self.cel = nn.CrossEntropyLoss()
#         self.optimizer = SAM(self.parameters(), torch.optim.SGD, lr=0.001, momentum=0.9)
#
#     def forward(self, x):
...
class Detector(nn.Module):
    def __init__(
        ...
        """
        super().__init__()
        optimizer_cfg = model_cfg.get('OPTIMIZER', {})  # 获取 'OPTIMIZER' 部分的内容
        self.optimizer_method = optimizer_cfg.get('OPTIMIZER_METHOD', 'adam')
        self.base_lr = optimizer_cfg.get('BASE_LR', 0.0002)
        self.adam_betas = optimizer_cfg.get('ADAM_BETAS', [0.9, 0.999])
        self.eps = optimizer_cfg.get('EPS', 0.00000001)
        self.weight_decay = optimizer_cfg.get('WEIGHT_DECAY', 0.000001)
        self.cel = nn.CrossEntropyLoss()
