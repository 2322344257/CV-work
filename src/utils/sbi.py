...
class SBI_Dataset(Dataset):
    def __init__(self, phase='train', image_size=224, n_frames=8):
        ...
        # print(f"初始化 SBI_Dataset，阶段：{phase}")
        image_list, label_list = init_ff(phase, 'frame', n_frames=n_frames)
        # print(f"构建的图像文件路径：{image_list}")
        # print(f"构建的标签文件路径：{label_list}")
        path_lm = '/landmarks/'
        label_list = [label_list[i] for i in range(len(image_list)) if os.path.isfile(
            image_list[i].replace('/frames/', path_lm).replace('.png', '.npy')) and os.path.isfile(
